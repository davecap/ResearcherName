...
class DataHandler:
    global_dtd_dir = os.path.join("entrez", "DTDs")
...
""" % (filename, filename, url, self.global_dtd_dir, filename)
            warnings.warn(message)
            try:
                handle = urlfetch.fetch(url=url)
                handle = StringIO.StringIO(handle.content)
            except IOError:
