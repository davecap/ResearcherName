...
E-utilities.""", UserWarning)
    ...
    try:
        if post:
            #HTTP POST            
            handle = urlfetch.fetch(url=cgi, payload=options, method=urlfetch.POST)
        else:
            #HTTP GET
            cgi += "?" + options
            handle = urlfetch.fetch(url=cgi, method=urlfetch.GET)
    except urlfetch.Error, exception:
        raise exception

    return StringIO.StringIO(handle.content)
